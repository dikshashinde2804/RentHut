...
const allowedOrigins = [
  "https://renthut.vercel.app",
  "http://localhost:3000",
  "https://rentora-server.vercel.app",
...
app.get("/allListings", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch lisitng" });
  }
});
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "Error creating listing" });
    }
  }
);
...
app.get("/getBooking/:id", async (req, res) => {
  try {
    ...
    const bookings = await booking
      ...
      .populate({
        path: "listingId",
        populate: { path: "owner", model: "User", select: "username email" },
      });
...
app.delete("/cancelBooking/:bookingId", async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: "Booking canceled and listing status updated",
    });
  } catch (error) {
...
app.post("/createBooking/:id", userVerification, async (req, res) => {
  ...
  try {
    ...
    if (existingListing.status === "Occupied") {
      return res
        .status(400)
        .json({ success: false, message: "Listing is already occupied" });
    }
    ...
    const options = {
      ...
      data: {
        ...
        order_meta: {
          ...
          notify_url:
            "https://webhook.site/c6844494-0754-44e1-a412-0795334b980e",
        },
    ...
    axios
      ...
      .catch(function (error) {
        ...
        return res
          ...
          .json({
            success: false,
            message: "Error initiating payment session",
          });
      });
  } catch (err) {
...
app.get("/verify/:orderId/:userId/:id", async (req, res) => {
  ...
  if (!userId || !listingId) {
    return res
      .status(400)
      .json({ error: "User ID and Listing ID are required." });
  }
  ...
  try {
    ...
    if (orderStatus === "PAID") {
      ...
      return res.json({
        success: false,
        status: "ACTIVE",
        message: "Payment in process. Please wait.",
      });
    } else {
      ...
      return res.json({
        success: false,
        status: "FAILED",
        message: "Payment failed. Booking not completed.",
      });
    }
  } catch (error) {
    console.log(error);
